...
export class DisplayConfigState {
  ...
  get builtin_monitors() {
    return this.monitors.filter(monitor => monitor.is_builtin);
  }

  get active_monitors() {
    let connectors = new Set();

    for (let logical_monitor of this.logical_monitors) {
      for (let monitor of logical_monitor.monitors) {
        connectors.add(monitor[0]);
      }
    }

    return this.monitors.filter(monitor => connectors.has(monitor.connector));
  }
  ...
  get_logical_monitors_for(connectors) {
    let connector_set = new Set(connectors);
    let seen = new Set();
    let matches = [];

    for (let logical_monitor of this.logical_monitors) {
      for (let monitor of logical_monitor.monitors) {
        let connector = monitor[0];
        if (!connector_set.has(connector) || seen.has(logical_monitor)) {
          continue;
        }

        matches.push(logical_monitor);
        seen.add(logical_monitor);
      }
    }
  ...
  get_logical_monitor_size(logical_monitor, transform = logical_monitor.transform) {
    if (logical_monitor.monitors.length === 0) {
      return { width: 0, height: 0 };
    }

    let connector = logical_monitor.monitors[0][0];
    let monitor = this.get_monitor(connector);
    if (monitor === null) {
      return { width: 0, height: 0 };
    }

    let width = Math.round(monitor.current_mode_width / logical_monitor.scale);
    let height = Math.round(monitor.current_mode_height / logical_monitor.scale);
